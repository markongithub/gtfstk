GTFSTK
********
GTFSTK is a Python 3.5 tool kit that analyzes `General Transit Feed Specification (GTFS) <https://en.wikipedia.org/wiki/GTFS>`_ data in memory without a database.
...
8.0.0, 2017-04-15
-----------------
- Finally knuckled down and wrote a GTFS validator: ``validators.py``.  It's basic, easy to read, and, thanks to Pandas, fast.  It checks `this 31 MB Southeast Queensland feed <http://transitfeeds.com/p/translink/21/20170310>` in 23 seconds on my 2.8-GHz-processor-16-GB-memory computer.  With the same computer and feed, `Google's GTFS validator <https://github.com/google/transitfeed>` takes 420 seconds. That's about 18 times slower. However, the Google validator performs more checks beyond the GTFS than ``validators.py``.
