...
class TestFeed(unittest.TestCase):
    ...
    #         num_routes = len(f['tmp'].unique())
    #         self.assertEqual(rs.shape[1], 8)
    #         self.assertEqual(rs.shape[0], num_routes)
    ...
    def test_get_routes_time_series(self):
        ...
        f = pd.merge(trips_stats, feed.get_trips_activity(dates))
        f = f[f[dates].sum(axis=1) > 0]
        # Its values should be data frames of the correct shape
        for split_directions in [False, True]:
            rts = feed.get_routes_time_series(trips_stats, dates, 
              split_directions=split_directions)
            # Should be a dictionary
            self.assertIsInstance(rts, dict)
            if split_directions:
                f['tmp'] = f['route_id'] + '-' +\
                  f['direction_id'].map(str)
            else:
            ...
            for key in rts:
                print(key)
                self.assertIsInstance(rts[key], pd.core.frame.DataFrame)
                self.assertEqual(rts[key].shape[0], 24*60)
                self.assertEqual(rts[key].shape[1], num_routes)
